...
async function checkConfiguration() {
  return new Promise((resolve) => {
    try {
      ...
      console.error("Error in checkConfiguration:", e);
      resolve(false);
    }
  });
}
...
function getCurrentText(element) {
  try {
    ...
    console.error("Error getting current text:", e);
    return "";
  }
}
...
function createGhostTextElement() {
  try {
    ...
    console.error("Error creating ghost text element:", e);
    return null;
  }
}
...
function showSuggestionPanel(improvedText) {
  try {
    ...
    console.error("Error showing suggestion panel:", e);
  }
}
...
function hideSuggestionPanel() {
  try {
    ...
    console.error("Error hiding suggestion panel:", e);
  }
}
...
function acceptImprovement() {
  try {
    ...
    console.error("Error accepting improvement:", e);
  }
}
...
function handleFocusIn(e) {
  try {
    ...
    console.error("Error in focusin handler:", e);
  }
}
        ...
        chrome.runtime.sendMessage(
          ...
          (response) => {
            try {
              ...
              console.error("Caught error in fetchCompletion callback:", callbackError);
            }
          }
        );
      } catch (e) {
        console.error("Error in debounced input handler:", e);
      }
    }, 500); // 500ms debounce
  } catch (e) {
      ...
      chrome.runtime.sendMessage(
        ...
        (response) => {
          try {
            ...
            console.error("Caught error in improveText callback:", callbackError);
          }
        }
      );
      return;
